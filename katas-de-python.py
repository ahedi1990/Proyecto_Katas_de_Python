...
try:
    lista = [2, 4, 6, 8, 10]
    print(f'El promedio es {calcula_promedio(lista)}')
    
    lista_vacia = []
    # llama a la la excepción
    print(f'El promedio es {calcula_promedio(lista_vacia)}')  
...
print(f"Tu edad es {obtener_edad()}.")
...
def palabras_con_letra(lista_palabras, letra):
    return list(filter(lambda w: w.startswith(letra), lista_palabras))

lista_palabras = ['taza', 'vaso', 'abrigo', 'bolsa', 'ordenador', 'mochila', 'bolso', 'blusa'] 
resultado = palabras_con_letra(lista_palabras, 'b')      
...
def palabras_mas_largas(frase, n):
    palabras = frase.split()
    return list(filter(lambda w: len(w) > n, palabras))
...
def buscar_nombre_en_lista(lista_nombres):
...
try:
    buscar_nombre_en_lista(lista_nombres)
...
def procesar_texto(texto, opcion, *args):
    ...
    if opcion == "contar_palabras":
        ...
        if len(args) < 2:
            raise ValueError("Se requieren dos palabras: la original y la nueva.")
...
#------------------------------------------------------------------
texto = "Estas opciones y son otras funciones que tenemos que definir primero y llamar dentro de y la función y procesar_texto"
# Contar
print(procesar_texto(texto, "contar_palabras"))  
# Reemplazar
print(procesar_texto(texto, "reemplazar_palabras", "y", "cambio").lower())  
# Eliminar
print(procesar_texto(texto, "eliminar_palabra", "y").lower()) 
...
print(calcular_area("circulo", (20,)))

print(calcular_area("triangulo", (7, 15)))
